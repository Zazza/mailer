...
class Model_Ui extends Engine_Model {
	...
	public function getInfo($loginSession) {
		...
		$sql = "SELECT id, `login`, pass, name, soname
                FROM users 
	...
	public function editUser($uid, $login, $name, $soname) {
		$sql = "UPDATE users SET `login` = :login, `name` = :name, `soname` = :soname WHERE id = :id LIMIT 1";
		$res = $this->registry['db']->prepare($sql);
		$param = array(":id" => $uid, ":login" => $login, ":name" => $name, ":soname" => $soname);
		$res->execute($param);
	}
